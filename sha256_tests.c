...
/*		sha256_init((uint32_t*)hash);

		block_pos = 0;
		block_pos = sha256_add_input(tests[i].mesg, tests[i].len, (uint32_t*)hash,
		                             block, block_pos);

		sha256_finish(tests[i].len, block, block_pos, (uint32_t*)hash);
*/
		sha256(tests[i].mesg, tests[i].len, (uint32_t*)hash);
	...
	// Test with string of 1,000,000 letter 'a's
	sha256_init((uint32_t*)hash);

	block_pos = 0;
	for (i = -20000; i; i++)
		block_pos = sha256_add_input("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
		                             50, (uint32_t*)hash, block, block_pos);
	sha256_finish(1000000, block, block_pos, (uint32_t*)hash);

	bin2hex(hash, hex_buf);

	const char* BIG_TEST_HASH = "cdc76e5c9914fb9281a1c7e284d73e67f1809a48a497200e046d39ccc7112cd0";

	if (str_cmp(hex_buf, BIG_TEST_HASH)) {
		printf("Big test failed:\nExpected: %s\nReceived: %s\n", BIG_TEST_HASH, hex_buf);
		return 1;
	}
